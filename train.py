...
writer = SummaryWriter('logs_market1501_batchhard_softplus/person_reid')
images_path = '../../market_1501/'
os.makedirs('checkpoints_market1501_batchhard_softplus', exist_ok = True)
n_classes = 18
n_samples = 4
...
def run_epoch(data_loader, model, optimizer, split = 'train', epoch_count = 0):
	...
	for batch_id, (imgs, labels) in enumerate(train_loader):
		...
		batch_loss, non_zero_losses = batch_hard_online_triplet_loss(labels, embeddings, margin = 0.2, squared = False)
		optimizer.zero_grad()
		...
		writer.add_scalar('Batch_Hard_Online_Triplet_Loss', batch_loss.item(), iter_count)
...
def fit(train_loader, model, optimizer, n_epochs):
	...
	for epoch in range(n_epochs):
		...
		torch.save({'state_dict': model.cpu().state_dict()}, 'checkpoints_market1501_batchhard_softplus/model_epoch_' + str(epoch + 1) + '.pth')
